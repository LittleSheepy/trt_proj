...
namespace ObjDet {
	...
	ILayer* YOLOV4::convBnMish(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input, int outch, int ksize, int s, int p, int linx) {
	...
	ILayer* YOLOV4::convBnLeaky(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input, int outch, int ksize, int s, int p, int linx) {
		Weights emptywts{ DataType::kFLOAT, nullptr, 0 };
		IConvolutionLayer* conv1 = network->addConvolutionNd(input, outch, DimsHW{ ksize, ksize }, weightMap["module_list." + std::to_string(linx) + ".Conv2d.weight"], emptywts);
		assert(conv1);
		conv1->setStrideNd(DimsHW{ s, s });
	...
	ICudaEngine* YOLOV4::createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
	...
	void YOLOV4::APIToModel(unsigned int maxBatchSize, IHostMemory** modelStream) {
		// Create builder
	...
	void YOLOV4::doInference(IExecutionContext& context, float* input, float* output, int batchSize) {
	...
	std::vector<yolov4::Detection> YOLOV4::predict(cv::Mat& img) {
		...
		doInference(*m_context, data, this->output, BATCH_SIZE);
		auto end = std::chrono::system_clock::now();
		std::cout << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() << "ms" << std::endl;
		std::vector<std::vector<yolov4::Detection>> batch_res(1);
		for (int b = 0; b < 1; b++) {
			auto& res = batch_res[b];
			yolov4::nms(res, &this->output[0]);
		}
