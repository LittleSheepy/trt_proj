...
namespace nvinfer1
{
    YoloV5LayerPlugin::YoloV5LayerPlugin(int classCount, int netWidth, int netHeight, int maxOut, const std::vector<yolov5::YoloKernel>& vYoloKernel)
    {
    ...
    {
        return sizeof(mClassCount) + sizeof(mThreadCount) + sizeof(mKernelCount) + sizeof(yolov5::YoloKernel) * mYoloKernel.size() + sizeof(mYoloV5NetWidth) + sizeof(mYoloV5NetHeight) + sizeof(mMaxOutObject);
    }
    ...
    IPluginV2IOExt* YoloV5PluginCreator::createPlugin(const char* name, const PluginFieldCollection* fc)
    {
        ...
        std::vector<yolov5::YoloKernel> kernels(fc->fields[1].length);
        memcpy(&kernels[0], fc->fields[1].data, kernels.size() * sizeof(yolov5::YoloKernel));
        YoloV5LayerPlugin* obj = new YoloV5LayerPlugin(class_count, input_w, input_h, max_output_object_count, kernels);
